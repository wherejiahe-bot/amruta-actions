...
def dp_align(sim_matrix, skip_cost=0.6):
    """
    Bertalign 式 DP 句对齐。
    sim_matrix: (M, N) array-like, cosine similarity in [0, 1]
...
# ========== Deduplication check before output ==========
# Check both Chinese and English for duplicates
zh_count = {}
en_count = {}
for en, zh in pairs:
    ...
    if en_str:
        en_count[en_str] = en_count.get(en_str, 0) + 1
    if zh_str:
        zh_count[zh_str] = zh_count.get(zh_str, 0) + 1

has_dup = False

dup_zh = {zh: cnt for zh, cnt in zh_count.items() if cnt > 1}
if dup_zh:
    has_dup = True
    print(f'[DEDUP WARNING] Found {len(dup_zh)} duplicate ZH sentence(s):')
    for zh, cnt in dup_zh.items():
        en_list = [en for en, z in pairs if z.strip() == zh]
        print(f'  ZH: {zh[:80]}...')
        print(f'  Count: {cnt}, mapped EN sentences:')
        for i, e in enumerate(en_list):
            print(f'    [{i}] {e[:80]}')

dup_en = {en: cnt for en, cnt in en_count.items() if cnt > 1}
if dup_en:
    has_dup = True
    print(f'[DEDUP WARNING] Found {len(dup_en)} duplicate EN sentence(s):')
    for en, cnt in dup_en.items():
        zh_list = [z for e, z in pairs if e.strip() == en]
        print(f'  EN: {en[:80]}...')
        print(f'  Count: {cnt}, mapped ZH sentences:')
        for i, z in enumerate(zh_list):
            print(f'    [{i}] {z[:80] if z else "(empty)"}')

if not has_dup:
    print('[DEDUP] No duplicates found. OK.')
