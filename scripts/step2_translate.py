...
def aliyun_translate_title(text):
    """Translate text via Aliyun MT API - inline implementation, no module dependency"""
    ...
    try:
        ...
        if translated and translated != str(text).strip():
            print('[aliyun_title] OK: "' + str(text).strip()[:40] + '" -> "' + translated[:40] + '"')
            return translated
        else:
            print('[aliyun_title] Empty result for: ' + str(text).strip()[:40])
...
def extract_title_cn_from_pairs(pairs_list, en_title):
    ...
    if not title_words:
        return None
    # Try keyword match on pairs that have Chinese
    ...
    if best_zh:
        print('[title] from pairs: "' + best_zh + '" (score=' + str(round(best_score,2)) + ')')
        return best_zh
    # If all pairs have empty Chinese, try Aliyun
    print('[title] no match in pairs, trying Aliyun...')
    t = aliyun_translate_title(en_title)
    if t:
        print('[title] from Aliyun: "' + t + '"')
        return t
    return None
