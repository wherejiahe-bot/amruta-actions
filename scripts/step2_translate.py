...
ima_kb_doc_title = ""  # 被选为翻译来源的 IMA KB 文档标题（主人验证用，因为 sahaja.live 被墙打不开）
ima_kb_source_url = ""  # IMA KB 文档的 source URL（sahaja.live 链接）
...
def parse_ima_bilingual_md(full_text):
    ...
    支持三种格式：
      ...
      3. separated（新增）：所有英文段落先列完，所有中文段落紧随其后（EN-all-first / ZH-all-after）
...
def search_ima_kb(query_text, phase_name, date_str=None):
    ...
    try:
            ...
            try:
                ...
                if src_url and date_str:
                    ...
                    if src_md and src_md == article_md:
                        ...
                        sahaja_link = src_url
                        ima_kb_doc_title = doc.get("title", "")
                        ima_kb_source_url = src_url
                        print(f'[translate_article] IMA source link (verified): {sahaja_link[:80]}')
                        print(f'[translate_article] IMA KB doc title: {ima_kb_doc_title[:80]}')
        ...
        if fallback_text and fallback_parsed_pairs:
            ...
            # Fallback - look for the first doc that had a valid title
            ima_kb_doc_title = docs_to_try[0].get("title", "") if docs_to_try else ""
            ima_kb_source_url = fallback_src_url
            if sahaja_link:
...
if ima_kb_doc_title:
    ...
    html += "<strong>翻译来源：</strong>来源于 sahaja.live 官方翻译（IMA 知识库）<br>"
    html += "文档标题：" + ima_kb_doc_title.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
    if ima_kb_source_url:
        html += "<br>来源：<a href=\"" + ima_kb_source_url + "\" style=\"color:#999;\">" + ima_kb_source_url + "</a>"
    html += "</p>"
elif ima_translation_fail_reason:
    # ❌ IMA KB 搜索失败，使用机器翻译
    html += "<p style=color:#999;font-size:0.78em;margin:0 0 8px 0;padding:8px 10px;background:#fafafa;border-radius:6px;line-height:1.5;>"
    ...
    # 可能来自 sahaja.live WP API 或旧缓存（非 IMA KB 路径）— 这种情况如果没标注就不加说明
    pass
...
html += "<a href=\"" + second_link + "\" style=color:#aaa;>" + second_link + "</a>"
html += "</p>"
...
# 写入 IMA KB 文档标题（供验收/审核用）
with open("/tmp/ima_kb_doc_title.txt", "w", encoding="utf-8") as f:
    f.write(ima_kb_doc_title)

# 写入 IMA KB source URL（真实的 sahaja.live 链接，不是 amruta.today）
with open("/tmp/ima_kb_source_url.txt", "w", encoding="utf-8") as f:
    f.write(ima_kb_source_url or sahaja_link or "")
