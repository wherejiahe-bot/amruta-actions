  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        else reject(new Error(`GET ${path} HTTP ${res.statusCode}: ${body.substring(0,200)}`));
      });
    });
    req.on('error', reject);
    req.end();
...
function githubDelete(path, sha, message) {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({ message, sha, branch: BRANCH });
    const options = {
      hostname: 'api.github.com',
      path: `/repos/${OWNER}/${REPO}/contents/${path}`,
      method: 'DELETE',
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) resolve(JSON.parse(body));
        else reject(new Error(`DELETE ${path} HTTP ${res.statusCode}: ${body.substring(0,200)}`));
      });
    });
    req.on('error', reject);
    req.write(data);
...
async function main(){
  ...
  let articles = JSON.parse(Buffer.from(af.content,'base64').toString('utf-8'));
  const articlesSha = af.sha;

  console.log(`Current year: ${curYear}`);
  console.log(`Found ${articles.length} articles\n`);

  // Identify renames needed
  const renames = articles
    .map(a => {
      const [origYear, mm, dd] = a.date.split('-');
      const newDate = `${curYear}-${mm}-${dd}`;
      return { ...a, newDate, needsRename: newDate !== a.date };
    });

  // Process renames
  for (const a of renames) {
    if (!a.needsRename) {
      console.log(`${a.date} -> same, skip`);
      continue;
    }
    process.stdout.write(`${a.date} -> ${a.newDate}... `);
    try {
      // 1. Fetch old file
      const oldPath = `daily/${a.date}.html`;
      const newPath = `daily/${a.newDate}.html`;
      const oldFile = await githubGet(oldPath);
      if (!oldFile) { console.log('old file not found, skip'); continue; }
      const html = Buffer.from(oldFile.content,'base64').toString('utf-8');
      const pairs = [];
      const re = /<div class="pair">\s*<div class="en-text">([\s\S]*?)<\/div>\s*<div class="zh-text">([\s\S]*?)<\/div>\s*<\/div>/g;
      let m;
      while ((m = re.exec(html)) !== null) {
      ...
      const srcM = html.match(/Source:\s*<a href="([^"]+)"/);
      const sourceUrl = srcM ? srcM[1] : (a.source||'');

      // 2. Create new file with new date
      const newHtml = buildDailyHtml({ date: a.newDate, title: a.title, titleCn: a.titleCn, pairs, sourceUrl });
      // Check if new file already exists
      const existingNew = await githubGet(newPath);
      await githubPut(newPath, newHtml, `rename: ${a.date} -> ${a.newDate}`, existingNew ? existingNew.sha : undefined);
      await sleep(600);

      // 3. Delete old file
      await githubDelete(oldPath, oldFile.sha, `rename: remove old ${a.date}`);
      await sleep(600);
  ...
  // Update articles.json — replace dates
  const updatedArticles = renames.map(a => {
    const updated = { ...a };
    delete updated.needsRename;
    if (a.needsRename) {
      updated.date = a.newDate;
      // update month/year fields
      const [y, mm, dd] = a.newDate.split('-');
      const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
      updated.year = y;
      updated.month = monthNames[parseInt(mm)-1];
    }
  ...
  await githubPut('articles.json', JSON.stringify(updatedArticles, null, 2),
    'rename: update articles.json with new dates', articlesSha);
  console.log('articles.json updated');

  // Rebuild index.html
  console.log('Rebuilding index.html...');
  const existingIndex = await githubGet('index.html');
  const indexHtml = buildIndexHtml(updatedArticles);
  await githubPut('index.html', indexHtml, 'rename: rebuild index with new dates', existingIndex ? existingIndex.sha : undefined);
  console.log('index.html updated');

  console.log('\n=== Final article dates ===');
  updatedArticles.forEach(a => console.log(`  ${a.date} - ${a.title}`));
  console.log('\nDone!');
}
