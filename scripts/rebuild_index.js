...
async function main() {
  ...
  const articles = JSON.parse(Buffer.from(existing.content, 'base64').toString('utf-8'));

  console.log('Preview of date badges:');
  const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const sorted = [...articles].sort((a,b) => (b.pushDate||b.date).localeCompare(a.pushDate||a.date));
  sorted.forEach(a => {
    const pushYear = (a.pushDate||a.date).split('-')[0];
    const dp = a.date.split('-');
    const mon = monthNames[parseInt(dp[1])-1];
    const day = parseInt(dp[2]);
    console.log(`  ${mon} ${day} ${pushYear}  ← speech: ${a.date}, push: ${a.pushDate||'?'}`);
  });
  ...
  await githubPut('index.html', indexHtml, 'fix: date badge shows speech month/day + push year', indexSha);
